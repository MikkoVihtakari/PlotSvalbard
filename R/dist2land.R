...
#' @details If \code{geodesic_distances = TRUE}, the function uses the \code{\link[rgeos]{gDistance}} function to calculate closest distances between coordinates in \code{x} and a specified SpatialPolygonsDataframe object. The spatial object (map) can be specified using the \code{\link[=basemap]{map.type}} argument. If \code{geodesic_distances = FALSE}, the \code{\link[geosphere]{dist2Line}} is used to calculate similar distances assumming an elliptical Earth. The \code{\link[geosphere]{dist2Line}} function is presumably more exact, especially for pan-Arctic maps, but considerably slower.  
...
#' d_panarctic <- transform_coord(d_panarctic, lon = "Lon", lat = "Lat", map.type = "arctic50", bind = TRUE)
#'
#' basemap("arctic50") +
...
dist2land <- function(x, lon.col = "longitude", lat.col = "latitude", map.type = "arctic50", bind = TRUE, dist.col = "dist", geodesic_distances = FALSE) {
  ...
  if(geodesic_distances) {
    ...
    if(proj4string(pp) != proj4string(land)) stop("Cannot convert projections correctly.")
  }
