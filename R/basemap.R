...
ggplot(data=X$Land, aes(x=long, y=lat)) + geom_polygon(data = X$Land, aes(x = long, y = lat, group = group), fill = land.col, color = "black", size = 0.1) + geom_polygon(data = X$Glacier, aes(x = long, y = lat, group = group), fill = gla.col, color = "black", size = 0.1) + geom_polygon(data = X$Holes, aes(x=long, y=lat, group = group), fill = land.col, color = "black", size = 0.1) + geom_line(data = X$Grid$lat, aes(x = long.utm, y=lat.utm, group = ID), color = grid.col, size = 0.1) + geom_line(data = X$Grid$lon, aes(x=long.utm, y=lat.utm, group = ID), color = grid.col, size = 0.1) + scale_x_continuous(name = "Longitude (decimal degrees)", breaks = X$Grid$lon.breaks$utm, labels = X$Grid$lon.breaks$deg) + scale_y_continuous(name = "Latitude (decimal degrees)", breaks = X$Grid$lat.breaks$utm, labels = X$Grid$lat.breaks$deg) + coord_fixed(xlim = c(X$Grid$boundaries$long.utm[1], X$Grid$boundaries$long.utm[2]), ylim = c(X$Grid$boundaries$lat.utm[1], X$Grid$boundaries$lat.utm[2]), expand = FALSE) + theme_map()
