...
print.basemapLimits <- function(x, ...) {
  ...
  cat("Decimal degree:", sep = "\n")
  print(format(round(x$bound_dd, 3), justify = "left"), row.names = TRUE)
  cat(NULL, sep = "\n")
  cat("UTM:", sep = "\n")
  print(format(x$bound_utm, justify = "left"), row.names = TRUE)
  cat(NULL, sep = "\n")
  cat("Contains the boundaries above as a shapefile: ")
