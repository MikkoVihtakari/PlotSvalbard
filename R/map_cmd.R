...
map_cmd <- function(command, alternative = FALSE) {
  switch(command,
    ...
    bathy_pb = 'geom_polygon(data = bathy, aes(x = long, y = lat, group = group, fill = depth), show.legend = bathy.legend) + scale_fill_manual(name = "Depth (m)", values = colorRampPalette(c("#F7FBFF", "#DEEBF7", "#9ECAE1", "#4292C6", "#08306B"))(nlevels(bathy$depth)))',
    bathy_pg = 'geom_polygon(data = bathy, aes(x = long, y = lat, group = group, fill = depth), show.legend = bathy.legend) + scale_fill_grey("Depth (m)", start = 1, end = 0.7)',
