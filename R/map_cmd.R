...
map_cmd <- function(command, alternative = FALSE) {
  switch(command,
    ...
    currents_utm = if(alternative) {
      'geom_path(data = cur, aes(x = long, y = lat, group = group, size = size, color = type), alpha = current.alpha, arrow = arrow(type = "open", angle = 15, ends = "last", length = unit(0.3, "lines")), show.legend = current.legend) + scale_color_manual(name = "Current\ntype", values = c("Arctic" = arc.col, "Atlantic" = atl.col), guide = guide_legend(order = 2, override.aes = list(fill = NA))) + scale_size(name = "Current\nsize", range = c(LS(0.5),LS(2)), breaks = 1:5, labels = c("Intermittent", "Minor", "Medium", "Major", "Main"), guide = guide_legend(order = 3, override.aes = list(fill = NA)))'
