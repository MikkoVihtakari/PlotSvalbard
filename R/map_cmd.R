...
map_cmd <- function(command) {
  switch(command,
    ...
    bathy = 'geom_polygon(data = bathy, aes(x = long, y = lat, group = group, fill = depth), show.legend = bathy_legend) + scale_fill_manual(name = "Depth (m)", values = colorRampPalette(c("#F7FBFF", "#DEEBF7", "#9ECAE1", "#4292C6", "#08306B"))(nlevels(bathy$depth)))',
