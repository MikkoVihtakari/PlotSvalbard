...
map_cmd <- function(command) {
  switch(command,
    ...
    bathy_pb = 'geom_polygon(data = bathy, aes(x = long, y = lat, group = group, fill = depth), show.legend = bathy_legend) + scale_fill_manual(name = "Depth (m)", values = colorRampPalette(c("#F7FBFF", "#DEEBF7", "#9ECAE1", "#4292C6", "#08306B"))(nlevels(bathy$depth)))',
    bathy_pg = 'geom_polygon(data = bathy, aes(x = long, y = lat, group = group, fill = depth), show.legend = bathy_legend) + scale_fill_grey("Depth (m)", start = 1, end = 0.7)',
    bathy_cg = 'geom_polygon(data = bathy, aes(x = long, y = lat, group = group, color = depth), show.legend = bathy_legend, fill = NA, color = "grey", size = size.land)',
    bathy_cb = 'geom_polygon(data = bathy, aes(x = long, y = lat, group = group, color = depth), show.legend = bathy_legend, fill = NA, size = size.land) + scale_color_manual(name = "Depth (m)", values = colorRampPalette(c("#F7FBFF", "#DEEBF7", "#9ECAE1", "#4292C6", "#08306B"))(nlevels(bathy$depth)))',
    ...
    defs_polar = 'geom_path(data = X$Grid$lat[X$Grid$lat$ID == levels(X$Grid$lat$ID)[which.min(as.numeric(gsub("[[:alpha:]]", "", levels(X$Grid$lat$ID))))],], aes(x = lon.utm, y=lat.utm, group = ID), color = border.col.land, size = size.land) + coord_fixed() + theme_void(base_size = base_size)',
