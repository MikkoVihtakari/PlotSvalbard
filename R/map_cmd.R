...
map_cmd <- function(command, alternative = FALSE) {
  switch(command,
    ...
    currents_utm = if(alternative) {
      'geom_path(data = atl, aes(x = long, y = lat, group = group, size = size), color = atl.col, alpha = current.alpha, arrow = arrow(type = "closed", angle = 15, ends = "last", length = unit(0.3, "lines")), show.legend = legends) + geom_path(data = arc, aes(x = long, y = lat, group = group, size = size), color = arc.col, alpha = current.alpha, arrow = arrow(type = "closed", angle = 15, ends = "last", length = unit(0.3, "lines")), show.legend = legends) + scale_size(name = "Current\nsize", range = c(LS(0.5),LS(2)))'
