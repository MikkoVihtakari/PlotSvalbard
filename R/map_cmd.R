...
map_cmd <- function(command, alternative = FALSE) {
  switch(command,
    ...
    bathy_pb = 'geom_polygon(data = bathy, aes(x = long, y = lat, group = group, fill = depth), show.legend = bathy.legend, color = bathy.border.col, size = bathy.size) + scale_fill_manual(name = "Depth (m)", values = colorRampPalette(c("#F7FBFF", "#DEEBF7", "#9ECAE1", "#4292C6", "#08306B"))(nlevels(bathy$depth)), guide = guide_legend(order = 1, override.aes = list(colour = NA)))',
    bathy_pg = 'geom_polygon(data = bathy, aes(x = long, y = lat, group = group, fill = depth), show.legend = bathy.legend, color = bathy.border.col, size = bathy.size) + scale_fill_grey("Depth (m)", start = 1, end = 0.7, guide = guide_legend(order = 1, override.aes = list(colour = NA)))',
    ...
    currents_utm = if(alternative) {
      'geom_path(data = cur, aes(x = long, y = lat, group = group, size = size, color = type), alpha = current.alpha, arrow = arrow(type = "open", angle = 15, ends = "last", length = unit(0.3, "lines")), show.legend = current.legend, linejoin = "mitre") + scale_color_manual(name = "Current\ntype", values = c("Arctic" = arc.col, "Atlantic" = atl.col), guide = guide_legend(order = 2, override.aes = list(fill = NA))) + scale_size(name = "Current\nsize", range = c(LS(0.5), LS(3)), breaks = sort(unique(cur$size)), labels = c("Intermittent", "Minor", "Medium", "Major", "Main"), guide = guide_legend(order = 3, override.aes = list(fill = NA)))'
    ...
    defs_polar = 'geom_path(data = X$Grid$lat[X$Grid$lat$ID == levels(X$Grid$lat$ID)[which.min(as.numeric(gsub("[[:alpha:]]", "", levels(X$Grid$lat$ID))))],], aes(x = lon.utm, y=lat.utm, group = ID), color = land.border.col, size = land.size) + coord_fixed() + theme_void(base_size = base_size)',
