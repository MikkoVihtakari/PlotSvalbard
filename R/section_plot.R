...
#' @details Creates an oceanographic section plot from a data frame using ggplot2. Returns either an interpolated section or a bubble plot.
...
#' @examples # Bubble plots
#' data("chlorophyll")
#' section_plot(df = chlorophyll[grepl("KpN.|Kb[0-4]", chlorophyll$Station),], x = "lon", y = "From", z = "Chla")
#' 
...
#' ## Logarithmic y axis
#' section_plot(ctd_rijpfjord, x = "dist", y = "pressure", z = "temp", bottom = "bdepth", interpolate = TRUE, log_y = TRUE)
#' ## zbreaks have to be defined to plot contours
#' section_plot(ctd_rijpfjord, x = "dist", y = "pressure", z = "temp", bottom = "bdepth", interpolate = TRUE, log_y = TRUE, zbreaks = c(-1.8, 0, 1, 3))
#' @export
...
# Fix: contour label size, y_log axis scale, zbreaks not defined contours, add flexible color scales, add option to define the sampling range/location
section_plot <- function(df, x, y, z, bottom = NULL, interpolate = FALSE, interp_method = "mba", log_y = FALSE, xlab = "Distance", ylab = "Depth", zlab = "Variable", ybreaks = waiver(), xbreaks = waiver(), zbreaks = waiver(), xlim = NULL, ylim = NULL, zlim = NULL, zrange = c(1, 6), add_bottom = NULL, legend.position = "right", base_size = 10) {
...
if(log_y) {
  ...
  if(class(ybreaks) == "waiver") {
    ybreaks <- pretty(pretty(10^pretty(range(dt$y)) - 10))
...
if(interpolate) {
  ...
  ggplot() + 
    ...
    geom_contour(data = dt, aes(x = x, y = y, z = z), color = "white", size = LS(0.5), breaks = zbreaks) + 
    directlabels::geom_dl(data = dt, aes(x = x, y = y, z = z, label = ..level..), 
      method = list("bottom.pieces", cex = 0.4, vjust = 0.5), 
      stat = "contour", color = "white", breaks = zbreaks) + 
    geom_segment(data = samples, aes(x = x, xend = x, y = min, yend = max), size = LS(0.5), color = "grey", linetype = 2) + 
    scale_fill_continuous(type = "viridis", name = zlab, na.value = "white", limits = zlim, oob = scales::squish) + 
    scale_colour_continuous(type = "viridis", name = zlab, na.value = "white", limits = zlim, oob = scales::squish) + {
